...
BOOL DetourXS::Create(const LPVOID lpFuncOrig, const LPVOID lpFuncDetour,  LPVOID *callback)
{
	DWORD dwProt;
	...
	// Write the trampoline address before it gets called throug the patch
	if (callback)
		*callback = m_trampoline.data();
...
LPVOID DetourXS::RecurseJumps(LPVOID lpAddr)
{
	...
	if(*lpbAddr == 0xFF && *(lpbAddr + 1) == 0x25)
	{
		...
		{
			lpDest = *reinterpret_cast<LPVOID**>( lpbAddr + (intptr_t)*reinterpret_cast<int *>(lpbAddr + 2) + 6 );
		}
		else
		{
