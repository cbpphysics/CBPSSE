...
public:
    ...
    const std::unordered_map<std::string, std::string> & Section(std::string section) const;
...
inline const std::unordered_map<std::string, std::string>& INIReader::Section(std::string section) const
{
...
{
    try {
        return _values.at(section).count(name) ? _values.at(section).at(name) : default_value;
    }
    catch (...) {
